...
function initCalendar() {
  calendar = new FullCalendar.Calendar(document.getElementById('calendar'), {
    ...
    eventClick: (info) => {
        ...
        // Показуємо/ховаємо кнопку видалення (тільки технік або автор протягом 15 хв)
        const diff = (Date.now() - (props.createdAt || 0)) / 1000 / 60;
        const canDelete = currentUser.level !== 'teacher' || (props.creator === sessionStorage.getItem('st_token') && diff <= 15);
...
function confirmBooking() {
    ...
    if (isBlock) {
        ...
        const sClass = document.getElementById('eventClass').value; // Нове
        const count = document.getElementById('eventCount').value; // Нове
...
function updateStatusBar() {
    const now = new Date();
    const current = calendar.getEvents().find(e => now >= new Date(e.start) && now < new Date(e.end));
    const bar = document.getElementById('statusBar');
    if (current) {
        bar.style.background = current.extendedProps.type === 'block' ? '#6B7280' : '#EF4444';
        bar.textContent = `🔴 ЗАРАЗ: ${current.title}`;
    } else {
